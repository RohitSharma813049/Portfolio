...
function CategoryGridComponent({ initialCategories }: CategoryGridProps) {
  ...
  const totalPages = useMemo(() => {
    return Math.ceil(filteredAndSortedCategories.length / ITEMS_PER_PAGE) || 1;
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        >
          <div className="sticky top-8 bg-white border border-[#EAEAEA] rounded-2xl p-6 shadow-sm">
            ...
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#16325C] mb-4">
                Sort Categories
              </h4>
                ...
                {["A-Z", "Z-A", "Most Projects", "Least Projects"].map((sortOption) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm ${
                        sortBy === sortOption
                          ? "font-semibold text-[#0B1B3D]"
                          : "text-[#666] group-hover:text-[#0B1B3D]"
                      }`}
                    >
                      {sortOption}
                    </span>
...
export default memo(CategoryGridComponent);
