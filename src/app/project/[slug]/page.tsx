    ...
    <main className="min-h-screen bg-white pb-24">
      ...
      <header className="flex justify-between items-center py-6 px-8 max-w-7xl mx-auto border-b border-[#EAEAEA]">
        ...
        <nav className="flex gap-6 items-center">
          <Link href="/" className="text-sm font-medium hover:text-[#666666] transition">Back to Projects</Link>
        </nav>
      </header>
        ...
        <div className="flex flex-col md:flex-row gap-12 items-start">
          <div className="flex-1">
            {project.categories && project.categories.length > 0 && (
              <span className="inline-block py-1 px-3 rounded-full bg-[#f4f4f4] text-xs font-semibold uppercase tracking-wider mb-4 text-[#666666]">
            ...
            <div className="flex flex-wrap gap-4">
              <a href={project.bookDemoUrl || "#"} className="bg-[#111111] text-white px-6 py-3 rounded-xl text-sm font-medium hover:bg-[#333] transition shadow-soft">
                Book Demo
              </a>
              <a href={project.requestCostUrl || "#"} className="border border-[#EAEAEA] px-6 py-3 rounded-xl text-sm font-medium hover:bg-[#f4f4f4] transition">
                Enquiry for Cost
              </a>
              {project.livePreviewUrl && (
                <a href={project.livePreviewUrl} target="_blank" rel="noreferrer" className="border border-[#EAEAEA] px-6 py-3 rounded-xl text-sm font-medium hover:bg-[#f4f4f4] transition">
                  Live Preview
                </a>
              )}
            </div>
          ...
          <div className="w-full md:w-1/3 aspect-square bg-[#f8f9fa] rounded-[24px] border border-[#EAEAEA] flex items-center justify-center p-8 overflow-hidden">
            {project.featureImage ? (
               <img src={project.featureImage} alt={project.name} className="w-full h-full object-cover rounded-xl" />
            ) : (
               <div className="w-full h-full bg-gradient-to-br from-indigo-50 to-blue-100 rounded-xl flex items-center justify-center">
      ...
      <section className="max-w-4xl mx-auto px-8 mt-24 text-center">
        ...
        <p className="text-[#666666] mb-8">Book a demo or request pricing to get started with your deployment.</p>
        <a href={project.enquiryUrl || "#"} className="bg-[#111111] inline-block text-white px-8 py-4 rounded-xl font-medium hover:bg-[#333] transition shadow-soft">
          Talk to an Expert
        </a>
      </section>
    </main>
